...
knitr::kable(party_incumbent_promote, digits = 3, col.names = c("Party", "Incumbency","Promotion"))

```

Democratic incumbents had the highest average promotion rate
...
plot_q3<-fb_ads|>
  ...
  ggplot(aes(impressions_millions))+geom_histogram(binwidth = 1)
plot_q3

```
The graph is skewed right and the majority of house candidates had less than 10 million impressions
...
top_15<- fb_ads|>
  filter(office=="US House")|>
  arrange(desc(impressions_millions))|>head(15)

fb_top_plot<-ggplot(top_15,aes(impressions_millions,reorder(cand_name_last,impressions_millions),fill=party))+geom_col()+scale_fill_manual(values= c(R = "red", D = "blue"))+labs(title = "Top 15 US House Candidates by Facebook Ad Impressions", x = "Impressions (in millions)",y = "Candidate Last Name")
