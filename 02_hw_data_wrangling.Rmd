...
incumbency_table <- fb_ads |>
  group_by(incumbency) |>
  summarize(num_candidates = n(), .groups = 'drop')
if (ncol(incumbency_table) != 2) 
  stop("Wrong number of columns. Should be 2.")
  
  incumbency_table <- knitr::kable(incumbency_table)
...
fb_top_plot <- ggplot(top_15, aes(x = impressions_millions, y = reorder(cand_name_last, impressions_millions))) +
  geom_col(aes(fill = party)) +  # Correctly placed
